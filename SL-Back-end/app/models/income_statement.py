...
class IncomeStatement(Base):
    ...
    # Primary Key
    is_id = Column(BigInteger, primary_key=True, autoincrement=True, comment="손익계산서 항목 고유 ID")
    ...
    thstrm_amount = Column(BigInteger, nullable=True, comment="당기 금액 (원) - 해당 분기")
    thstrm_add_amount = Column(BigInteger, nullable=True, comment="당기 누적 금액 (원) - 연초부터 누적")
    ...
    frmtrm_add_amount = Column(BigInteger, nullable=True, comment="전기 누적 금액 (원)")
    ...
    # Relationships
    financial_statement = relationship("FinancialStatement", back_populates="income_statements")

    # Indexes
    __table_args__ = (
        # 재무제표별 계정과목 조회 최적화
        Index('idx_income_statements_stmt_account', 'stmt_id', 'account_nm'),

        # 정렬 순서 조회
        Index('idx_income_statements_stmt_ord', 'stmt_id', 'ord'),

        {"comment": "손익계산서 테이블 - 수익/비용/이익"}
    )

    def __repr__(self):
        return f"<IncomeStatement(id={self.is_id}, stmt_id={self.stmt_id}, account={self.account_nm})>"
