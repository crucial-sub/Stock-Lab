# 백테스트 실행 과정 상세 가이드

## 📋 목차
1. [백테스트 요청부터 완료까지 전체 플로우](#1-전체-플로우)
2. [각 단계별 상세 설명](#2-각-단계별-상세-설명)
3. [데이터베이스 테이블 Write 과정](#3-데이터베이스-테이블-write-과정)
4. [메모리 vs DB 저장](#4-메모리-vs-db-저장)
5. [에러 처리 및 상태 관리](#5-에러-처리-및-상태-관리)

---

## 1. 전체 플로우

```
┌─────────────────────────────────────────────────────────────┐
│                    사용자 요청                                │
│  POST /api/v1/backtest                                      │
│  {                                                          │
│    buy_conditions: [...],                                   │
│    sell_conditions: [...],                                  │
│    start_date: "2023-01-01",                               │
│    end_date: "2024-01-01",                                 │
│    initial_capital: 100000000,                             │
│    commission_rate: 0.00015,                               │
│    slippage: 0.001                                         │
│  }                                                          │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│              1. 백테스트 ID 생성                              │
│  backtest_id = uuid4()                                      │
│  예: "a3f5c2d8-9b4a-4e1c-8d7f-2a3b4c5d6e7f"               │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│         2. BacktestEngine 초기화                             │
│  engine = BacktestEngine(db)                               │ 
│  - tax_rate 설정 (0.23% 고정)                              │
│  - commission_rate, slippage를 파라미터로 받음              │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│         3. 데이터 로딩 (READ from DB)                        │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 3-1. 주가 데이터 로드                                 │  │
│  │   - 테이블: stock_prices + companies                │  │
│  │   - 기간: start_date - 365일 ~ end_date             │  │
│  │   - 컬럼: stock_code, date, OHLCV, market_cap 등   │  │
│  │   - 반환: pandas DataFrame (메모리 저장)             │  │
│  └──────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 3-2. 재무 데이터 로드                                 │  │
│  │   - 테이블: financial_statements +                  │  │
│  │            income_statements + balance_sheets       │  │
│  │   - 기간: start_date - 180일 ~ end_date             │  │
│  │   - 컬럼: 매출액, 순이익, 자본총계, 자산총계 등       │  │
│  │   - 반환: pandas DataFrame (메모리 저장)             │  │
│  └──────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 3-3. 벤치마크 데이터 생성                             │  │
│  │   - 현재: 더미 데이터 생성 (추후 실제 지수 연동)      │  │
│  │   - KOSPI/KOSDAQ 일별 가격 데이터                    │  │
│  └──────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│         4. 팩터 계산 (메모리 연산)                            │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 날짜별 반복 계산 (각 거래일마다)                      │  │
│  │                                                      │  │
│  │ For each trading_day in [start_date ~ end_date]:    │  │
│  │   ┌────────────────────────────────────────────┐    │  │
│  │   │ 4-1. 가치 팩터                              │    │  │
│  │   │   - PER, PBR, EV/EBITDA, DIV_YIELD        │    │  │
│  │   │   - 데이터: price + financial              │    │  │
│  │   └────────────────────────────────────────────┘    │  │
│  │   ┌────────────────────────────────────────────┐    │  │
│  │   │ 4-2. 수익성 팩터                            │    │  │
│  │   │   - ROE, ROA                              │    │  │
│  │   │   - 데이터: financial                      │    │  │
│  │   └────────────────────────────────────────────┘    │  │
│  │   ┌────────────────────────────────────────────┐    │  │
│  │   │ 4-3. 성장성 팩터                            │    │  │
│  │   │   - REVENUE_GROWTH, EARNINGS_GROWTH       │    │  │
│  │   │   - 데이터: financial (YoY 비교)           │    │  │
│  │   └────────────────────────────────────────────┘    │  │
│  │   ┌────────────────────────────────────────────┐    │  │
│  │   │ 4-4. 모멘텀 팩터                            │    │  │
│  │   │   - 1M, 3M, 6M, 12M 모멘텀                │    │  │
│  │   │   - 데이터: price (과거 가격 비교)          │    │  │
│  │   └────────────────────────────────────────────┘    │  │
│  │   ┌────────────────────────────────────────────┐    │  │
│  │   │ 4-5. 변동성 팩터                            │    │  │
│  │   │   - 60일 변동성 (연환산)                   │    │  │
│  │   │   - 데이터: price (일별 수익률)            │    │  │
│  │   └────────────────────────────────────────────┘    │  │
│  │   ┌────────────────────────────────────────────┐    │  │
│  │   │ 4-6. 유동성 팩터                            │    │  │
│  │   │   - 평균 거래대금, 회전율                  │    │  │
│  │   │   - 데이터: price (거래량, 상장주식수)      │    │  │
│  │   └────────────────────────────────────────────┘    │  │
│  │                                                      │  │
│  │ 반환: factor_df (메모리)                             │  │
│  │   - 각 날짜별 모든 종목의 팩터 값                     │  │
│  │   - 정규화 (Z-Score) 및 순위 계산 포함               │  │
│  └──────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│         5. 포트폴리오 시뮬레이션 (메모리 연산)                 │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 초기 설정                                             │  │
│  │   - cash_balance = initial_capital (100,000,000원)  │  │
│  │   - holdings = {} (빈 딕셔너리)                      │  │
│  │   - trades = [] (거래 내역 리스트)                   │  │
│  │   - daily_snapshots = [] (일별 스냅샷)               │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                             │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 거래일별 반복 (각 거래일마다)                         │  │
│  │                                                      │  │
│  │ For each trading_day in trading_days:               │  │
│  │                                                      │  │
│  │   ┌────────────────────────────────────────────┐    │  │
│  │   │ 5-1. 리밸런싱 체크                          │    │  │
│  │   │   - MONTHLY: 매월 첫 거래일                │    │  │
│  │   │   - WEEKLY: 매주 월요일                    │    │  │
│  │   │   - DAILY: 매일                           │    │  │
│  │   │   - QUARTERLY: 분기별 첫 거래일            │    │  │
│  │   └────────────────────────────────────────────┘    │  │
│  │                                                      │  │
│  │   IF 리밸런싱 날짜:                                  │  │
│  │   ┌────────────────────────────────────────────┐    │  │
│  │   │ 5-2. 매도 실행                              │    │  │
│  │   │   For each holding in holdings:            │    │  │
│  │   │     - 매도 조건 체크:                       │    │  │
│  │   │       • 손절: -10% 이하                    │    │  │
│  │   │       • 익절: +20% 이상                    │    │  │
│  │   │       • 보유기간: 60일 초과                │    │  │
│  │   │                                            │    │  │
│  │   │     IF 매도 조건 충족:                     │    │  │
│  │   │       1) 매도가 = 현재가                   │    │  │
│  │   │       2) 수수료 = 매도금액 × 0.015%        │    │  │
│  │   │       3) 거래세 = 매도금액 × 0.23%         │    │  │
│  │   │       4) 실현손익 계산                     │    │  │
│  │   │       5) trades 리스트에 추가              │    │  │
│  │   │       6) cash_balance 증가                │    │  │
│  │   │       7) holdings에서 제거                 │    │  │
│  │   └────────────────────────────────────────────┘    │  │
│  │                                                      │  │
│  │   ┌────────────────────────────────────────────┐    │  │
│  │   │ 5-3. 매수 종목 선정                         │    │  │
│  │   │   1) 거래 가능 종목 필터링:                 │    │  │
│  │   │      - volume > 0                          │    │  │
│  │   │      - close_price > 0                     │    │  │
│  │   │      - 이미 보유 중이지 않은 종목           │    │  │
│  │   │                                            │    │  │
│  │   │   2) 팩터 기반 스코어링:                    │    │  │
│  │   │      For each buy_condition:               │    │  │
│  │   │        - PER < 15 → score +1               │    │  │
│  │   │        - ROE > 10 → score +1               │    │  │
│  │   │        - MOMENTUM_3M > 0 → score +1        │    │  │
│  │   │                                            │    │  │
│  │   │   3) 상위 N개 종목 선택 (max_positions)     │    │  │
│  │   └────────────────────────────────────────────┘    │  │
│  │                                                      │  │
│  │   ┌────────────────────────────────────────────┐    │  │
│  │   │ 5-4. 포지션 사이징                          │    │  │
│  │   │   - EQUAL_WEIGHT (동일 가중):              │    │  │
│  │   │     각 종목 = cash × 0.95 / num_positions  │    │  │
│  │   │                                            │    │  │
│  │   │   - MARKET_CAP (시총 가중):                │    │  │
│  │   │     각 종목 비중 = 해당 종목 시총 / 전체   │    │  │
│  │   │                                            │    │  │
│  │   │   - RISK_PARITY (리스크 패리티):           │    │  │
│  │   │     변동성 역수 비례 배분                  │    │  │
│  │   └────────────────────────────────────────────┘    │  │
│  │                                                      │  │
│  │   ┌────────────────────────────────────────────┐    │  │
│  │   │ 5-5. 매수 실행                              │    │  │
│  │   │   For each buy_candidate:                  │    │  │
│  │   │     1) 슬리피지 적용:                       │    │  │
│  │   │        execution_price = close × 1.001     │    │  │
│  │   │                                            │    │  │
│  │   │     2) 매수 수량 계산:                      │    │  │
│  │   │        quantity = allocation / exec_price  │    │  │
│  │   │                                            │    │  │
│  │   │     3) 수수료 계산:                         │    │  │
│  │   │        commission = amount × 0.015%        │    │  │
│  │   │                                            │    │  │
│  │   │     4) 잔고 확인:                           │    │  │
│  │   │        IF amount + commission > cash:      │    │  │
│  │   │          SKIP (매수 불가)                  │    │  │
│  │   │                                            │    │  │
│  │   │     5) trades 리스트에 추가                 │    │  │
│  │   │     6) cash_balance 감소                   │    │  │
│  │   │     7) holdings에 추가                     │    │  │
│  │   └────────────────────────────────────────────┘    │  │
│  │                                                      │  │
│  │   ┌────────────────────────────────────────────┐    │  │
│  │   │ 5-6. 일별 스냅샷 저장                       │    │  │
│  │   │   - portfolio_value = cash + 보유주식가치  │    │  │
│  │   │   - cash_balance                           │    │  │
│  │   │   - invested_amount                        │    │  │
│  │   │   - holdings (복사본)                      │    │  │
│  │   │   - trade_count                            │    │  │
│  │   │                                            │    │  │
│  │   │   → daily_snapshots 리스트에 추가          │    │  │
│  │   └────────────────────────────────────────────┘    │  │
│  │                                                      │  │
│  │ 반환: portfolio_result (메모리)                      │  │
│  │   - trades: 전체 거래 내역                           │  │
│  │   - daily_snapshots: 일별 포트폴리오 상태             │  │
│  │   - final_holdings: 최종 보유 종목                   │  │
│  │   - final_cash: 최종 현금 잔고                       │  │
│  │   - rebalance_dates: 리밸런싱 실행 날짜               │  │
│  └──────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│         6. 통계 계산 (메모리 연산)                            │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 6-1. 수익률 지표                                      │  │
│  │   - total_return: 총 수익률 (%)                      │  │
│  │   - annualized_return: CAGR                         │  │
│  │     = (final_value / initial)^(1/years) - 1         │  │
│  └──────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 6-2. 리스크 지표                                      │  │
│  │   - max_drawdown (MDD):                             │  │
│  │     = max((portfolio_value - cummax) / cummax)      │  │
│  │                                                      │  │
│  │   - volatility: 일별 수익률 표준편차 × √252          │  │
│  │                                                      │  │
│  │   - downside_volatility: 음수 수익률만의 표준편차     │  │
│  └──────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 6-3. 리스크 조정 수익률                               │  │
│  │   - sharpe_ratio:                                   │  │
│  │     = (annualized_return - rf) / volatility         │  │
│  │                                                      │  │
│  │   - sortino_ratio:                                  │  │
│  │     = (annualized_return - rf) / downside_vol       │  │
│  │                                                      │  │
│  │   - calmar_ratio:                                   │  │
│  │     = annualized_return / max_drawdown              │  │
│  └──────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 6-4. 거래 통계                                        │  │
│  │   - total_trades: 총 거래 횟수                       │  │
│  │   - winning_trades: 수익 거래 수                     │  │
│  │   - losing_trades: 손실 거래 수                      │  │
│  │   - win_rate: 승률 (%)                              │  │
│  │   - avg_win: 평균 수익 (%)                          │  │
│  │   - avg_loss: 평균 손실 (%)                         │  │
│  │   - profit_loss_ratio: 손익비                       │  │
│  └──────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│         7. 결과 포맷팅 (메모리 연산)                          │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 7-1. 일별 성과 (DailyPerformance)                    │  │
│  │   For each daily_snapshot:                          │  │
│  │     - date, portfolio_value, cash_balance           │  │
│  │     - daily_return, cumulative_return               │  │
│  │     - drawdown, trade_count                         │  │
│  └──────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 7-2. 월별 성과 (MonthlyPerformance)                  │  │
│  │   Group by (year, month):                           │  │
│  │     - return_rate: 월 수익률                         │  │
│  │     - win_rate: 월 승률                             │  │
│  │     - trade_count: 월 거래 수                        │  │
│  └──────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 7-3. 연도별 성과 (YearlyPerformance)                 │  │
│  │   Group by year:                                    │  │
│  │     - return_rate: 연 수익률                         │  │
│  │     - max_drawdown: 연 최대낙폭                      │  │
│  │     - sharpe_ratio: 연 샤프비율                      │  │
│  └──────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 7-4. 거래 내역 (TradeRecord)                         │  │
│  │   For each trade:                                   │  │
│  │     - trade_id, date, type (BUY/SELL)               │  │
│  │     - stock_code, quantity, price                   │  │
│  │     - commission, tax, profit, profit_rate          │  │
│  └──────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 7-5. 현재 보유 종목 (PortfolioHolding)               │  │
│  │   For each final_holding:                           │  │
│  │     DB에서 최신 가격 조회 (READ)                     │  │
│  │     - current_price, profit, profit_rate 계산        │  │
│  │     - weight: 포트폴리오 비중                        │  │
│  └──────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 7-6. 차트 데이터 생성                                 │  │
│  │   - dates: 날짜 배열                                 │  │
│  │   - portfolio_values: 포트폴리오 가치 배열            │  │
│  │   - cumulative_returns: 누적 수익률 배열             │  │
│  │   - drawdowns: 낙폭 배열                             │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                             │
│  반환: BacktestResult 객체 (메모리)                  │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│         8. 결과 저장 (현재 미구현, 추후 구현 예정)             │
│  ⚠️ 현재는 _save_result() 메서드가 비어있음                  │
│                                                             │
│  TODO: 다음 테이블들에 WRITE 예정                            │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 8-1. backtest_sessions 테이블                        │  │
│  │   INSERT:                                            │  │
│  │     - backtest_id (UUID)                            │  │
│  │     - backtest_name                                 │  │
│  │     - status (COMPLETED/FAILED)                     │  │
│  │     - created_at, completed_at                      │  │
│  │     - settings (JSON): commission, slippage 등       │  │
│  └──────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 8-2. backtest_conditions 테이블                      │  │
│  │   INSERT (bulk):                                    │  │
│  │     For each buy_condition:                         │  │
│  │       - backtest_id, condition_type (BUY/SELL)      │  │
│  │       - factor, operator, value                     │  │
│  └──────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 8-3. backtest_statistics 테이블                      │  │
│  │   INSERT:                                            │  │
│  │     - backtest_id                                   │  │
│  │     - total_return, annualized_return               │  │
│  │     - max_drawdown, volatility                      │  │
│  │     - sharpe_ratio, sortino_ratio, calmar_ratio     │  │
│  │     - total_trades, win_rate, profit_loss_ratio     │  │
│  └──────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 8-4. backtest_daily_snapshots 테이블                 │  │
│  │   INSERT (bulk, ~250 rows/year):                    │  │
│  │     For each daily_snapshot:                        │  │
│  │       - backtest_id, snapshot_date                  │  │
│  │       - portfolio_value, cash_balance               │  │
│  │       - daily_return, cumulative_return             │  │
│  │       - drawdown                                    │  │
│  └──────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 8-5. backtest_trades 테이블                          │  │
│  │   INSERT (bulk, 수백~수천 rows):                     │  │
│  │     For each trade:                                 │  │
│  │       - backtest_id, trade_date, trade_type         │  │
│  │       - stock_code, quantity, price                 │  │
│  │       - commission, tax, profit, profit_rate        │  │
│  └──────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 8-6. backtest_holdings 테이블                        │  │
│  │   INSERT (bulk):                                    │  │
│  │     For each final_holding:                         │  │
│  │       - backtest_id, stock_code                     │  │
│  │       - quantity, avg_price, current_price          │  │
│  │       - profit, profit_rate, weight                 │  │
│  └──────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│         9. 응답 반환                                         │
│  HTTP 200 OK                                                │
│  Content-Type: application/json                             │
│                                                             │
│  {                                                          │
│    "backtest_id": "a3f5c2d8-...",                          │
│    "backtest_name": "20250109_143022",                     │
│    "status": "COMPLETED",                                  │
│    "statistics": {                                         │
│      "total_return": 15.23,                                │
│      "annualized_return": 14.87,                           │
│      "max_drawdown": -12.45,                               │
│      "sharpe_ratio": 1.42,                                 │
│      ...                                                   │
│    },                                                      │
│    "daily_performance": [...],                             │
│    "monthly_performance": [...],                           │
│    "trades": [...],                                        │
│    "current_holdings": [...],                              │
│    "chart_data": {...}                                     │
│  }                                                         │
└─────────────────────────────────────────────────────────────┘
```

---

## 2. 각 단계별 상세 설명

### 2-1. 백테스트 ID 생성
```python
from uuid import uuid4

backtest_id = uuid4()
# 예: UUID('a3f5c2d8-9b4a-4e1c-8d7f-2a3b4c5d6e7f')
```

**특징:**
- UUID v4 (랜덤 생성)
- 글로벌하게 유니크
- 128비트 = 16바이트
- 문자열 표현: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"

---

### 2-2. 데이터 로딩 세부 과정

#### A. 주가 데이터 로드 (`_load_price_data`)
```sql
SELECT
    sp.company_id,
    c.stock_code,
    c.company_name AS stock_name,
    sp.trade_date AS date,
    sp.open_price,
    sp.high_price,
    sp.low_price,
    sp.close_price,
    sp.volume,
    sp.trading_value,
    sp.market_cap,
    sp.listed_shares
FROM stock_prices sp
JOIN companies c ON sp.company_id = c.company_id
WHERE
    sp.trade_date >= '2022-01-01'  -- start_date - 365일
    AND sp.trade_date <= '2024-01-01'  -- end_date
    AND sp.close_price IS NOT NULL
    AND sp.volume > 0
ORDER BY sp.trade_date, c.stock_code
```

**데이터 크기 예상:**
- 종목 수: ~2,500개 (KOSPI + KOSDAQ)
- 기간: 2년 = ~500 거래일
- 총 rows: 2,500 × 500 = 1,250,000 rows
- 메모리: ~100-200 MB (pandas DataFrame)

---

#### B. 재무 데이터 로드 (`_load_financial_data`)
```sql
-- 손익계산서
SELECT
    fs.company_id,
    c.stock_code,
    fs.bsns_year AS fiscal_year,
    fs.reprt_code AS report_code,
    fs.report_date,
    is.account_nm,
    is.thstrm_amount AS current_amount
FROM financial_statements fs
JOIN income_statements is ON fs.stmt_id = is.stmt_id
JOIN companies c ON fs.company_id = c.company_id
WHERE
    fs.report_date >= '2023-07-01'  -- start_date - 180일
    AND fs.report_date <= '2024-01-01'
    AND is.account_nm IN ('매출액', '영업이익', '당기순이익', ...)
```

**데이터 크기 예상:**
- 종목 수: ~2,500개
- 분기당 계정과목: ~8개
- 기간: 6개월 = ~2분기
- 총 rows: 2,500 × 8 × 2 = 40,000 rows
- 메모리: ~5-10 MB

---

### 2-3. 팩터 계산 예시

```python
# 날짜: 2023-05-15
# 종목: 삼성전자 (005930)

# 1. PER 계산
market_cap = 400_000_000_000_000  # 400조원
net_income = 30_000_000_000_000   # 30조원 (TTM)
PER = market_cap / net_income = 13.33

# 2. 모멘텀 계산
current_price = 70_000원
price_3m_ago = 65_000원
MOMENTUM_3M = (70_000 / 65_000 - 1) × 100 = 7.69%

# 3. 변동성 계산
daily_returns = [-0.5%, 1.2%, -0.8%, 0.3%, ...]  # 60일치
volatility = std(daily_returns) × √252 = 18.5%
```

---

### 2-4. 포트폴리오 시뮬레이션 예시

```python
# 2023-05-01 (리밸런싱 날짜)
cash_balance = 100,000,000원
holdings = {}

# 매수 종목 선정
buy_candidates = ['005930', '000660', '035420', ...]  # 20개

# 포지션 사이징 (EQUAL_WEIGHT)
per_stock = 100,000,000 × 0.95 / 20 = 4,750,000원

# 매수 실행
for stock in buy_candidates:
    current_price = get_price(stock, '2023-05-01')
    execution_price = current_price × 1.001  # 슬리피지
    quantity = 4,750,000 / execution_price
    commission = 4,750,000 × 0.00015

    trades.append({
        'type': 'BUY',
        'stock': stock,
        'quantity': quantity,
        'price': execution_price,
        'amount': 4,750,000,
        'commission': commission
    })

    cash_balance -= (4,750,000 + commission)
    holdings[stock] = {'quantity': quantity, 'avg_price': execution_price}

# 결과
cash_balance = 5,000,000원 (5%)
holdings = 20개 종목 보유
```

---

## 3. 데이터베이스 테이블 Write 과정

### 현재 상태: ❌ 미구현
```python
async def _save_result(self, backtest_id: UUID, result: BacktestResult):
    """결과 저장 (추후 구현)"""
    # 실제 구현시 데이터베이스에 결과 저장
    logger.info(f"Saving backtest result for {backtest_id}")
    # 여기에 실제 저장 로직 추가
    pass  # 현재 비어있음
```

### 구현 예정 테이블 스키마

#### 3-1. backtest_sessions 테이블
```sql
CREATE TABLE backtest_sessions (
    backtest_id UUID PRIMARY KEY,
    backtest_name VARCHAR(200) NOT NULL,
    user_id INTEGER,  -- 추후 사용자 인증 시
    status VARCHAR(20) NOT NULL,  -- PENDING/RUNNING/COMPLETED/FAILED
    created_at TIMESTAMP NOT NULL DEFAULT NOW(),
    started_at TIMESTAMP,
    completed_at TIMESTAMP,

    -- 설정 정보 (JSON)
    settings JSONB,  -- {rebalance_frequency, max_positions, ...}

    -- 기간 정보
    start_date DATE NOT NULL,
    end_date DATE NOT NULL,
    initial_capital DECIMAL(20,2) NOT NULL,

    -- 에러 정보
    error_message TEXT
);
```

**INSERT 예시:**
```python
session = BacktestSession(
    backtest_id=backtest_id,
    backtest_name="20250109_143022",
    status="COMPLETED",
    created_at=datetime.now(),
    completed_at=datetime.now(),
    settings={
        "rebalance_frequency": "MONTHLY",
        "max_positions": 20,
        "commission_rate": 0.00015,
        "slippage": 0.001
    },
    start_date=date(2023, 1, 1),
    end_date=date(2024, 1, 1),
    initial_capital=Decimal("100000000")
)
await db.add(session)
await db.commit()
```

---

#### 3-2. backtest_conditions 테이블
```sql
CREATE TABLE backtest_conditions (
    condition_id BIGSERIAL PRIMARY KEY,
    backtest_id UUID NOT NULL REFERENCES backtest_sessions(backtest_id) ON DELETE CASCADE,
    condition_type VARCHAR(10) NOT NULL,  -- BUY/SELL
    factor VARCHAR(50) NOT NULL,
    operator VARCHAR(10) NOT NULL,
    value DECIMAL(10,4) NOT NULL,
    weight DECIMAL(5,2),
    description TEXT
);

CREATE INDEX idx_backtest_conditions_backtest ON backtest_conditions(backtest_id);
```

**INSERT 예시:**
```python
# 매수 조건
conditions = [
    BacktestCondition(
        backtest_id=backtest_id,
        condition_type="BUY",
        factor="PER",
        operator="<",
        value=15.0
    ),
    BacktestCondition(
        backtest_id=backtest_id,
        condition_type="BUY",
        factor="ROE",
        operator=">",
        value=10.0
    ),
    # ... more conditions
]
await db.bulk_save_objects(conditions)
await db.commit()
```

---

#### 3-3. backtest_statistics 테이블
```sql
CREATE TABLE backtest_statistics (
    stat_id BIGSERIAL PRIMARY KEY,
    backtest_id UUID NOT NULL UNIQUE REFERENCES backtest_sessions(backtest_id) ON DELETE CASCADE,

    -- 수익률 지표
    total_return DECIMAL(10,4),
    annualized_return DECIMAL(10,4),
    benchmark_return DECIMAL(10,4),
    excess_return DECIMAL(10,4),

    -- 리스크 지표
    max_drawdown DECIMAL(10,4),
    volatility DECIMAL(10,4),
    downside_volatility DECIMAL(10,4),

    -- 리스크 조정 수익률
    sharpe_ratio DECIMAL(10,4),
    sortino_ratio DECIMAL(10,4),
    calmar_ratio DECIMAL(10,4),

    -- 거래 통계
    total_trades INTEGER,
    winning_trades INTEGER,
    losing_trades INTEGER,
    win_rate DECIMAL(5,2),
    avg_win DECIMAL(10,4),
    avg_loss DECIMAL(10,4),
    profit_loss_ratio DECIMAL(10,4),

    -- 자산 정보
    initial_capital DECIMAL(20,2),
    final_capital DECIMAL(20,2),
    peak_capital DECIMAL(20,2),

    trading_days INTEGER
);
```

---

#### 3-4. backtest_daily_snapshots 테이블
```sql
CREATE TABLE backtest_daily_snapshots (
    snapshot_id BIGSERIAL PRIMARY KEY,
    backtest_id UUID NOT NULL REFERENCES backtest_sessions(backtest_id) ON DELETE CASCADE,
    snapshot_date DATE NOT NULL,

    portfolio_value DECIMAL(20,2),
    cash_balance DECIMAL(20,2),
    invested_amount DECIMAL(20,2),

    daily_return DECIMAL(10,6),
    cumulative_return DECIMAL(10,6),
    drawdown DECIMAL(10,6),
    benchmark_return DECIMAL(10,6),

    trade_count INTEGER DEFAULT 0
);

CREATE INDEX idx_backtest_snapshots_backtest_date ON backtest_daily_snapshots(backtest_id, snapshot_date);
```

**데이터 크기:**
- 1년 백테스트 = ~250 rows
- 5년 백테스트 = ~1,250 rows

---

#### 3-5. backtest_trades 테이블
```sql
CREATE TABLE backtest_trades (
    trade_id BIGSERIAL PRIMARY KEY,
    backtest_id UUID NOT NULL REFERENCES backtest_sessions(backtest_id) ON DELETE CASCADE,

    trade_date DATE NOT NULL,
    trade_type VARCHAR(10) NOT NULL,  -- BUY/SELL
    stock_code VARCHAR(6) NOT NULL,
    stock_name VARCHAR(200),

    quantity INTEGER NOT NULL,
    price DECIMAL(15,2) NOT NULL,
    amount DECIMAL(20,2) NOT NULL,
    commission DECIMAL(15,2),
    tax DECIMAL(15,2),

    -- 매도 시에만
    profit DECIMAL(15,2),
    profit_rate DECIMAL(10,4),
    hold_days INTEGER,

    selection_reason TEXT
);

CREATE INDEX idx_backtest_trades_backtest ON backtest_trades(backtest_id);
CREATE INDEX idx_backtest_trades_date ON backtest_trades(backtest_id, trade_date);
```

**데이터 크기:**
- 월별 리밸런싱 × 20종목 × 2(매수+매도) = 40 trades/month
- 1년 = ~480 trades
- 5년 = ~2,400 trades

---

#### 3-6. backtest_holdings 테이블
```sql
CREATE TABLE backtest_holdings (
    holding_id BIGSERIAL PRIMARY KEY,
    backtest_id UUID NOT NULL REFERENCES backtest_sessions(backtest_id) ON DELETE CASCADE,

    stock_code VARCHAR(6) NOT NULL,
    stock_name VARCHAR(200),

    quantity INTEGER NOT NULL,
    avg_price DECIMAL(15,2) NOT NULL,
    current_price DECIMAL(15,2),

    value DECIMAL(20,2),
    profit DECIMAL(15,2),
    profit_rate DECIMAL(10,4),
    weight DECIMAL(5,2),

    buy_date DATE,
    hold_days INTEGER
);

CREATE INDEX idx_backtest_holdings_backtest ON backtest_holdings(backtest_id);
```

---

## 4. 메모리 vs DB 저장

### 현재 구현 (메모리 기반)
```
┌─────────────────────────────────────┐
│         메모리 (RAM)                 │
│                                     │
│  ┌──────────────────────────────┐  │
│  │ price_data (DataFrame)       │  │
│  │ ~200 MB                      │  │
│  └──────────────────────────────┘  │
│                                     │
│  ┌──────────────────────────────┐  │
│  │ financial_data (DataFrame)   │  │
│  │ ~10 MB                       │  │
│  └──────────────────────────────┘  │
│                                     │
│  ┌──────────────────────────────┐  │
│  │ factor_data (DataFrame)      │  │
│  │ ~50 MB                       │  │
│  └──────────────────────────────┘  │
│                                     │
│  ┌──────────────────────────────┐  │
│  │ portfolio_result (Dict)      │  │
│  │ ~5 MB                        │  │
│  │  - trades: List[Dict]        │  │
│  │  - daily_snapshots: List     │  │
│  │  - holdings: Dict            │  │
│  └──────────────────────────────┘  │
│                                     │
│  총 메모리 사용량: ~300 MB           │
└─────────────────────────────────────┘
```

**장점:**
- ✅ 빠른 계산 속도
- ✅ 복잡한 쿼리 불필요
- ✅ pandas/polars 최적화 활용

**단점:**
- ❌ 재시작 시 데이터 손실
- ❌ 대용량 백테스트 시 메모리 부족 가능
- ❌ 결과 영구 저장 불가

---

### 구현 예정 (하이브리드)
```
┌─────────────────────────────────────┐
│         메모리 (RAM)                 │
│  - 계산 중간 결과                    │
│  - DataFrame 연산                    │
│  - 임시 팩터 데이터                  │
└─────────────────────────────────────┘
             ↓ (완료 후)
┌─────────────────────────────────────┐
│    PostgreSQL (RDS)                 │
│                                     │
│  ┌──────────────────────────────┐  │
│  │ backtest_sessions            │  │
│  │ - 메타 정보, 상태             │  │
│  └──────────────────────────────┘  │
│                                     │
│  ┌──────────────────────────────┐  │
│  │ backtest_statistics          │  │
│  │ - 수익률, 리스크 지표         │  │
│  └──────────────────────────────┘  │
│                                     │
│  ┌──────────────────────────────┐  │
│  │ backtest_daily_snapshots     │  │
│  │ - 일별 포트폴리오 상태        │  │
│  └──────────────────────────────┘  │
│                                     │
│  ┌──────────────────────────────┐  │
│  │ backtest_trades              │  │
│  │ - 거래 내역                  │  │
│  └──────────────────────────────┘  │
└─────────────────────────────────────┘
```

**장점:**
- ✅ 결과 영구 저장
- ✅ 과거 백테스트 비교 가능
- ✅ 대시보드/분석 도구 연동
- ✅ 계산 중에는 메모리 활용 (빠름)

---

## 5. 에러 처리 및 상태 관리

### 5-1. 백테스트 상태 전이
```
PENDING → RUNNING → COMPLETED
                  ↘ FAILED
```

### 5-2. 에러 처리 예시
```python
async def run_backtest(...):
    try:
        # 1. 상태: RUNNING
        await self._update_backtest_status(backtest_id, "RUNNING")

        # 2. 데이터 로딩
        price_data = await self._load_price_data(...)
        if price_data.empty:
            raise ValueError("No price data found")

        # 3. 팩터 계산
        factor_data = await self._calculate_all_factors(...)

        # 4. 시뮬레이션
        portfolio_result = await self._simulate_portfolio(...)

        # 5. 상태: COMPLETED
        await self._update_backtest_status(backtest_id, "COMPLETED")

        return result

    except Exception as e:
        # 6. 상태: FAILED
        await self._update_backtest_status(
            backtest_id,
            "FAILED",
            error_message=str(e)
        )
        logger.error(f"Backtest failed: {e}", exc_info=True)
        raise
```

---

## 6. 성능 최적화 고려사항

### 6-1. 데이터베이스 쿼리 최적화
```python
# ✅ 좋은 예: 인덱스 활용
SELECT * FROM stock_prices
WHERE trade_date >= '2023-01-01'  -- idx_stock_prices_date_company 활용
  AND company_id IN (...)

# ❌ 나쁜 예: 풀스캔
SELECT * FROM stock_prices
WHERE YEAR(trade_date) = 2023  -- 함수 사용으로 인덱스 미활용
```

### 6-2. Bulk Insert
```python
# ✅ 좋은 예: Bulk insert (한 번에 1000개)
await db.bulk_save_objects(daily_snapshots)
await db.commit()

# ❌ 나쁜 예: 개별 insert (250번 커밋)
for snapshot in daily_snapshots:
    await db.add(snapshot)
    await db.commit()  # 너무 느림
```

### 6-3. 메모리 관리
```python
# 대용량 데이터 처리 시 청크 단위 처리
CHUNK_SIZE = 10000

for chunk in pd.read_sql(query, engine, chunksize=CHUNK_SIZE):
    # 청크별 처리
    process_chunk(chunk)
```

---

## 7. 요약

| 항목 | 현재 상태 | 구현 예정 |
|-----|----------|----------|
| **백테스트 ID** | ✅ UUID 생성 | - |
| **데이터 로딩** | ✅ DB READ (메모리) | - |
| **팩터 계산** | ✅ 메모리 연산 | - |
| **포트폴리오 시뮬레이션** | ✅ 메모리 연산 | - |
| **통계 계산** | ✅ 메모리 연산 | - |
| **결과 포맷팅** | ✅ Pydantic 모델 | - |
| **DB 저장** | ❌ 미구현 (pass) | ⏳ 구현 예정 |
| **상태 관리** | ❌ 미구현 | ⏳ 구현 예정 |
| **에러 핸들링** | ✅ Try-catch | ⏳ DB 로깅 추가 |

---

## 8. 다음 단계 (구현 필요)

1. **DB 저장 로직 구현**
   - `_save_result()` 메서드 완성
   - Bulk insert 최적화

2. **백테스트 상태 관리**
   - `backtest_sessions` 테이블에 상태 기록
   - PENDING → RUNNING → COMPLETED/FAILED

3. **백테스트 목록 조회 API**
   - 과거 백테스트 결과 조회
   - 페이지네이션

4. **백테스트 재조회 API**
   - `GET /api/v1/backtest/{backtest_id}`
   - DB에서 결과 불러오기

5. **백테스트 비교 기능**
   - 여러 백테스트 결과 비교
   - 차트 오버레이
