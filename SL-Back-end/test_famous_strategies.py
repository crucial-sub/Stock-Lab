...
FAMOUS_STRATEGIES = [
    ...
    },
    {
        ...
        "buy_conditions": [
            ...
            {"name": "B", "exp_left_side": "기본값({영업이익률})", "inequality": ">", "exp_right_side": 10}
        ],
        "buy_logic": "A AND B",
        "priority_factor": "ROE",
        "priority_order": "desc"
    },
    {
        ...
        "buy_conditions": [
            {"name": "A", "exp_left_side": "기본값({부채비율})", "inequality": "<", "exp_right_side": 50},
            {"name": "B", "exp_left_side": "기본값({자기자본비율})", "inequality": ">", "exp_right_side": 60}
        ],
        "buy_logic": "A AND B",
        "priority_factor": "부채비율",
        "priority_order": "asc"
    }
]
    ...
    try:
        async with AsyncSessionLocal() as db:
            ...
            if hasattr(engine, 'corporate_actions') and engine.corporate_actions:
                ...
                for ca in result["corporate_actions"]:
                    logger.warning(f"   - {ca['stock_code']}: {ca['event_date']} ({ca['change_rate']:+.1f}%)")
...
async def run_all_strategies():
    ...
    모든 유명 전략 백테스트 실행
    """
    # 1년 기간 설정 (2023-11-01 ~ 2024-10-31)
    end_date = "20241031"
    start_date = "20231101"

    logger.info("\n" + "="*80)
    logger.info("🎯 유명 전략 백테스트 시작")
    logger.info(f"   기간: {start_date} ~ {end_date} (1년)")
    ...
    logger.info("\n📈 전략별 수익률:")
    logger.info("-"*60)
    logger.info(f"{'전략명':<25} {'총수익률':>10} {'연수익률':>10} {'MDD':>10}")
    logger.info("-"*60)

    for r in results:
        if r["error"]:
            logger.info(f"{r['strategy_name']:<25} {'오류':>10}")
        else:
            perf = r["performance"]
            if perf:
                logger.info(
                    f"{r['strategy_name']:<25} "
                    f"{perf.get('total_return', 0):>9.2f}% "
                    f"{perf.get('annual_return', 0):>9.2f}% "
                    f"{perf.get('max_drawdown', 0):>9.2f}%"
                )
            else:
                logger.info(f"{r['strategy_name']:<25} {'데이터 없음':>10}")
