...
export function BuyConditionTabNew() {
    ...
    <>
          ...
          <SummaryPanel title="요약보기">
            <div className="space-y-6 text-sm">
              ...
              <SummarySection title="일반 조건">
                ...
                <SummaryItem
                  label="투자 시작일"
                  value={start_date ? start_date.replace(/(\d{4})(\d{2})(\d{2})/, "$1.$2.$3") : "-"}
                />
                <SummaryItem
                  label="투자 종료일"
                  value={end_date ? end_date.replace(/(\d{4})(\d{2})(\d{2})/, "$1.$2.$3") : "-"}
                />
                <SummaryItem label="수수료율" value={`${commission_rate}%`} />
              </SummarySection>

              {/* 매수 조건 */}
              <SummarySection title="매수 조건">
                <div>
                  <span className="text-text-muted">매수 조건식</span>
                  <div className="mt-1 space-y-0.5">
                    {buyConditions.map((c) => (
                      <div key={c.id} className="text-xs">
                        {c.id}: {getConditionExpression(c)}
                      </div>
                    ))}
                  </div>
                </div>
                {buy_logic && (
                  <SummaryItem label="논리 조건식" value={buy_logic} />
                )}
              </SummarySection>

              {/* 매수 비중 */}
              <SummarySection title="매수 비중">
                <SummaryItem label="종목당 비중" value={`${per_stock_ratio}%`} />
                <SummaryItem label="최대 보유" value={`${max_holdings}종목`} />
              </SummarySection>

              {/* 매수 방법 */}
              <SummarySection title="매수 방법">
                <SummaryItem label="가격 기준" value={buyCostBasisSelect} />
              </SummarySection>
            </div>
          </SummaryPanel>
        }
      >
        ...
        <div className="space-y-6 pb-12">
          {/* 일반 조건 설정 */}
          <div id="일반 조건 설정" className="bg-surface rounded-lg shadow-card p-8">
          ...
          {/* 매수 조건 설정 */}
          <div id="매수 조건 설정" className="bg-surface rounded-lg shadow-card p-8">
          ...
          {/* 매수 비중 설정 */}
          <div id="매수 비중 설정" className="bg-surface rounded-lg shadow-card p-8">
            <h2 className="text-2xl font-bold mb-6">
              <span className="text-brand-primary">매수</span>
          ...
          {/* 매수 방법 선택 */}
          <div id="매수 방법 선택" className="bg-surface rounded-lg shadow-card p-8">
            <h2 className="text-2xl font-bold mb-6">
              <span className="text-brand-primary">매수</span>
...
export default BuyConditionTabNew;
