 ...
 * - 오른쪽: 설정 요약 패널
 */
export function SellConditionTabNew() {
  // Zustand stores
  const {
    target_and_loss,
    setTargetAndLoss,
  ...
  const summary = (
    <SummaryPanel title="요약보기">
      <div className="space-y-6">
        ...
        <SummarySection
          title="목표가 / 손절가"
          titleColor={targetLossOpen ? "text-accent-primary" : "text-text-body"}
        >
          {targetLossOpen ? (
            <>
              {profitTargetEnabled && (
              ...
              {stopLossEnabled && (
                <>
                  <SummaryItem
                    label="손절가 (활성화)"
                    value={`매수가 대비 ${stopLoss}% 하락`}
                  />
                </>
              )}
              {!profitTargetEnabled && !stopLossEnabled && (
                <SummaryItem label="상태" value="비활성화" />
              )}
            </>
          ) : (
            <SummaryItem label="상태" value="비활성화" />
        ...
        <SummarySection
          title="보유 기간"
          titleColor={holdPeriodOpen ? "text-accent-primary" : "text-text-body"}
        >
          {holdPeriodOpen ? (
            <>
              <SummaryItem
              ...
              <SummaryItem
                label="매도 가격 기준"
                value={`${holdSellCostBasisSelect}, ${holdSellCostBasisValue}%`}
              />
            </>
          ) : (
            <SummaryItem label="상태" value="비활성화" />
        ...
        <SummarySection
          title="조건 매도"
          titleColor={
            conditionalSellOpen ? "text-accent-primary" : "text-text-body"
          }
        >
          {conditionalSellOpen ? (
            <>
              <SummaryItem
                label="매도 조건식 설정"
                value={sellConditions.map((c) => c.id).join(", ")}
              />
              <SummaryItem label="논리 조건식" value={sellLogic} />
              <SummaryItem
                label="매도 가격 기준"
                value={`${condSellCostBasisSelect}, ${condSellCostBasisValue}%`}
              />
            </>
          ) : (
            <SummaryItem label="상태" value="비활성화" />
          ...
          >
            <div
              className={`absolute w-[22px] h-[22px] bg-white rounded-full top-[3px] transition-all ${
                targetLossOpen ? "left-[25px]" : "left-[3px]"
              }`}
            />
          </button>
        </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  >
                    <div
                      className={`absolute w-[22px] h-[22px] bg-white rounded-full top-[3px] transition-all ${
                        profitTargetEnabled ? "left-[25px]" : "left-[3px]"
                      }`}
                    />
                  </button>
                </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  >
                    <div
                      className={`absolute w-[22px] h-[22px] bg-white rounded-full top-[3px] transition-all ${
                        stopLossEnabled ? "left-[25px]" : "left-[3px]"
                      }`}
                    />
                  </button>
                </div>
      ...
      <div className="bg-surface rounded-lg shadow-card p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          >
            <div
              className={`absolute w-[22px] h-[22px] bg-white rounded-full top-[3px] transition-all ${
                holdPeriodOpen ? "left-[25px]" : "left-[3px]"
              }`}
            />
          </button>
        </div>
      ...
      <div className="bg-surface rounded-lg shadow-card p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          >
            <div
              className={`absolute w-[22px] h-[22px] bg-white rounded-full top-[3px] transition-all ${
                conditionalSellOpen ? "left-[25px]" : "left-[3px]"
              }`}
            />
          </button>
        </div>
...
export default SellConditionTabNew;
