 ...
 * - 오른쪽: 설정 요약 패널
 */
export function TargetSelectionTabNew() {
  ...
  // 요약 패널 컨텐츠
  const summary = (
    <SummaryPanel title="요약보기">
      <div className="space-y-6">
        {/* 매매 대상 종목 */}
        <SummarySection title="매매 대상 종목" titleColor="text-accent-primary">
          <SummaryItem
            label="선택한 대상 종목"
            value={`${mockStocks.length}개 / ${mockStocks.length}개 (선택 / 전체)`}
          />
          <SummaryItem
            label="2539 종목"
            value={`${selectedIndustries.size > 0 ? Array.from(selectedIndustries).join(", ") : "미선택"}`}
          />
        </SummarySection>

        {/* 전체 종목 */}
        <SummarySection title="전체 종목">
          <SummaryItem label="3580 종목" value="" />
        </SummarySection>

        {/* 선택한 유니버스 */}
        <SummarySection title="선택한 유니버스">
          <div className="space-y-1">
            {Array.from(selectedIndustries).map((industry) => (
              <div key={industry} className="text-sm text-text-body">
                • {industry}
              </div>
            ))}
          </div>
        ...
        {/* 선택한 테마 */}
        <SummarySection title="선택한 테마">
          <SummaryItem label="선택한 테마 수" value={`${selectedThemes.size}개`} />
          <SummaryItem label="전체 테마 수" value={`${themeOptions.length}개`} />
        </SummarySection>

        {/* 선택한 세부 종목 */}
        <SummarySection title="선택한 세부 종목">
          <div className="space-y-1">
            {mockStocks.map((stock) => (
              <div key={stock.code} className="text-sm text-text-body">
                • {stock.name}
              </div>
            ))}
          </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          className="flex-1 px-4 py-3 bg-surface border border-border-default rounded-sm text-text-body placeholder:text-text-muted focus:outline-none focus:border-accent-primary"
        />
        <button
          type="button"
          className="px-4 py-3 bg-surface border border-border-default rounded-sm hover:bg-bg-muted transition-colors"
        >
...
export default TargetSelectionTabNew;
