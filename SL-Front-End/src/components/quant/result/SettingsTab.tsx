    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          <SettingSection title="전략 정보">
            ...
            {settings.strategyType && (
              <SettingItem
                label="전략 유형"
                value={formatStrategyType(settings.strategyType)}
              />
            )}
      ...
      <div className="bg-bg-surface rounded-lg shadow-card p-6 border-l-4 border-blue-500">
        ...
        {settings.factors.length === 0 ? (
          ...
          <div className="space-y-4">
            {/* Screening Factors */}
            {settings.factors.filter((f) => f.usageType === "SCREENING")
              .length > 0 && (
                ...
                <div className="space-y-2">
                  {settings.factors
                    ...
                    .map((factor, idx) => (
                      <div
                        key={idx}
                        className="text-sm text-text-body bg-bg-muted p-2 rounded"
                      >
                        <p className="font-medium">{factor.factorName}</p>
                        {factor.operator &&
                          factor.thresholdValue !== undefined && (
                            <p className="text-xs text-text-muted">
                              {formatOperator(factor.operator)}{" "}
                              {factor.thresholdValue}
                            </p>
                          )}
                      </div>
                    ))}
            ...
            {/* Ranking Factors */}
            {settings.factors.filter((f) => f.usageType === "RANKING").length >
              0 && (
            ...
            {/* Scoring Factors */}
            {settings.factors.filter((f) => f.usageType === "SCORING").length >
              0 && (
            ...
            {settings.tradingRules.map((rule, idx) => (
              <div key={idx} className="space-y-3">
                <SettingSection title={formatRuleType(rule.ruleType)}>
                  ...
                  {rule.rebalanceDay !== undefined &&
                    rule.rebalanceDay !== null && (
                      <SettingItem
                        label="리밸런싱 일"
                        value={`${rule.rebalanceDay}일`}
                      />
                    )}
                  {rule.positionSizing && (
                    <SettingItem
                  ...
                  {rule.maxPositions !== undefined &&
                    rule.maxPositions !== null && (
                      <SettingItem
                        label="최대 보유 종목"
                        value={`${rule.maxPositions}개`}
                      />
                    )}
                  {rule.minPositionWeight !== undefined &&
                    rule.minPositionWeight !== null && (
                      <SettingItem
                        label="최소 종목 비중"
                        value={`${(rule.minPositionWeight * 100).toFixed(2)}%`}
                      />
                    )}
                  {rule.maxPositionWeight !== undefined &&
                    rule.maxPositionWeight !== null && (
                      <SettingItem
                        label="최대 종목 비중"
                  ...
                  {rule.takeProfitPct !== undefined &&
                    rule.takeProfitPct !== null && (
                      <SettingItem
                        label="익절매 비율"
                        value={`${rule.takeProfitPct}%`}
                      />
                    )}
                  {rule.commissionRate !== undefined &&
                  ...
                  {rule.taxRate !== undefined && rule.taxRate !== null && (
                    <SettingItem
                      label="세금율"
                      value={`${(rule.taxRate * 100).toFixed(2)}%`}
                    />
                  )}
                </SettingSection>
              </div>
